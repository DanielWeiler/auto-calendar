 ...
 * @param {string} date - A string representing a Date object
 * @returns {Date} Returns the new Date object.
 */
export function addTimeToDate(time: string, date: string): Date {
  // The date string must be modified so that it can be used in a Date 
  // constructor to create a Date according to the user's time zone
  const modifiedDate = date.replaceAll('-', '/')

  const dateTime = new Date(modifiedDate)
